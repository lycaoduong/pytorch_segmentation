...
import timm
# from utils.loss_function import FocalLoss_Cls, DistillationLoss
...
class Attention(torch.nn.Module):
    def __init__(self, dim=384, key_dim=32, num_heads=8,
                 attn_ratio=4,
                 resolution=16): # Resolution Original is 7
        super().__init__()
    ...
    def forward_tokens(self, x):
        ...
        for idx, block in enumerate(self.network):
            ...
            if self.fork_feat and idx in self.out_indices:
                ...
                # layer_norm = nn.LayerNorm([64, 128, 128], device='cuda')
                # x_out = layer_norm(x)
                if x.ndim == 3:
                    B, W, H, C = x.shape[0], round(math.sqrt(x.shape[1])), round(math.sqrt(x.shape[1])), x.shape[2]
                    x = x.view(B, W, H, C)
                    x_out = norm_layer(x)
                else:
                    x_out = norm_layer(x.permute(0, 2, 3, 1))
                outs.append(x_out.permute(0, 3, 1, 2))
        if self.fork_feat:
            return outs
        return x

    # def forward_tokens_ly(self, x):
    #     outs = []
    #     B = 0
    #     C = 0
    #     H = 0
...
#     def __init__(self, model, teacher_model, loss_func_type='focal', device='cpu', alpha=None, gamma=2.0):
#         super().__init__()
#         self.model = model
#         if loss_func_type == 'focal':
#             self.criterion = FocalLoss_Cls(device=device, alpha=alpha, gamma=gamma, reduction='mean')
#         elif loss_func_type == 'dis':
#             base_criterion = FocalLoss_Cls(device=device, alpha=alpha, gamma=gamma, reduction='mean')
#             self.criterion = DistillationLoss(base_criterion, teacher_model, 'soft', 0.5, 1.0)
#
#     def forward(self, image, annot, **kwargs):
#         classification = self.model(image)
#         loss = self.criterion(inputs=image, outputs=classification, labels=annot)
#         return loss


if __name__ == '__main__':
    model = efficientformer_l1(fork_feat=True)
